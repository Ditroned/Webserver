...
int ResHandler::filexist(const char *fileName)
{
	...
	if (CheckifPathisFile(fileName) == true)
	{
		if ((access(fileName, F_OK) == 0))
		{
			...
			else if (MainServer.cgitype == "php" && MainServer.mp[".php"].find("/usr/bin/php-cgi") != std::string::npos)
			{
				MainServer.http = g.cgiExecute("/usr/bin/php-cgi", fileName, MainServer.postname, MainServer.postvalue, 0, MainServer.bando, MainServer.code);
				MainServer.code = 200;
				return MainServer.code;
			}
			return MainServer.code;
...
int ResHandler::POSTMethodes()
{
	...
	if (MainServer.path.substr(MainServer.path.find_last_of(".") + 1) == "php")
		MainServer.http = g.cgiExecute("/usr/bin/php", path.c_str(), MainServer.postname, MainServer.postvalue, 0, MainServer.bando, MainServer.code);
